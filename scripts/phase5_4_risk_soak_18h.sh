...
OUT="artifacts/phase5_4_risk_soak_18h.txt"
SUMMARY="artifacts/phase5_4_risk_soak_18h_summary.json"
BOT_LOG="var/phase5_4_risk_soak_18h_bot.log"

DURATION_SEC="${DURATION_SEC:-64800}"
...
RISK_STATS_PATH="${RISK_STATS_PATH:-var/phase5_4_risk_stats_soak_18h.json}"
RISK_STATE_PATH="${RISK_STATE_PATH:-var/phase5_4_risk_state_soak_18h.json}"
...
{"desired_state":"RUNNING","force_dry_run":true,"risk_mode":"SAFE","reason":"phase5_4_soak_18h"}
JSON
...
# Run in its own process group so cleanup can reliably terminate both the go
# wrapper and the compiled child binary.
...
echo "[soak] waiting for api..." >>"$OUT"
if ! wait_for_api; then
  echo "[soak] api not ready; last 200 lines of bot log:" >>"$OUT"
  tail -n 200 "$BOT_LOG" >>"$OUT" || true
  exit 1
fi
echo "[soak] api ready" >>"$OUT"
...
echo "[soak] enqueue bootstrap intents..." >>"$OUT"
sleep 2
if enqueue_intent "${POOL_ID}" "mock_rebalance"; then
  echo "[soak] enqueue ok #1" >>"$OUT"
else
  echo "[soak] enqueue failed #1" >>"$OUT"
fi
sleep 2
if enqueue_intent "${POOL_ID}" "mock_rebalance"; then
  echo "[soak] enqueue ok #2" >>"$OUT"
else
  echo "[soak] enqueue failed #2" >>"$OUT"
fi
...
while true; do
  ...
  if ! kill -0 "$BOT_PID" >/dev/null 2>&1; then
    echo "[soak] bot exited early elapsed_sec=$ELAPSED" >>"$OUT"
    wait "$BOT_PID" || true
    echo "[soak] last 200 lines of bot log:" >>"$OUT"
    tail -n 200 "$BOT_LOG" >>"$OUT" || true
    exit 2
  fi
